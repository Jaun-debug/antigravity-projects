...
def parse_wetu_json(itinerary_data):
    ...
    for day in days:
             ...
             for dest in day['Destinations']:
                  if dest.get('Pictures'):
                       for pic in dest['Pictures']:
                            if pic.get('PictureUrl'):
                                 dest_images.append(pic.get('PictureUrl'))
                                 
        import urllib.parse
        def clean_list(img_list, limit):
            unique = []
            for img in img_list:
        ...
        lodge_images = clean_list(lodge_images, 6)
        dest_images = clean_list(dest_images, 3)
        
        all_images.extend(lodge_images)
        all_images.extend(dest_images)
        
        # We store both so app.py can group them intelligently
        parsed_days.append({
            "day_range": day_range,
            "location": loc_name,
            "accommodation": acc_name,
            "description": desc,
            "lodge_images": lodge_images,
