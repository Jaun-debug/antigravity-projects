...
def run():
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch(
            headless=True,
            channel="chrome", # Use the system Chrome to avoid mach_port crashes
            args=[
                '--disable-crashpad',
                '--disable-mach-ports',
                '--disable-gpu',
        ...
        try:
            ...
            if not itinerary_data:
                ls_data = page.evaluate('() => window.localStorage.getItem("TEMP_RESULTS")')
                if ls_data:
                    itinerary_data = json.loads(ls_data)
                    print("Captured from localStorage TEMP_RESULTS")
